...
#             logger.info(f"병원 종류 필터링 후: {len(filtered)}개")
...
# #     def _fetch_hospital_data(self, district=None, name=None, hospital_type=None, limit=2000):
# #         url = f"{self.BASE_URL}/{self.api_key}/xml/TbHospitalInfo/1/{limit}/"
# #         params = {}
# #         if district:
    ...
    def _apply_filters(self, hospitals, district, hospital_name, hospital_type):
        ...
        if hospital_type:
            ...
            if len(filtered) == 0 and original_count > 0:
                type_distribution = {}
                for h in hospitals[:10]:  # 처음 10개만 확인
                    h_type = h.get("type", "정보없음")
                    type_distribution[h_type] = type_distribution.get(h_type, 0) + 1
                logger.info(f"사용 가능한 병원 타입들: {type_distribution}")
