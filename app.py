...
# 추가: 배치 대화 저장을 위한 큐와 설정
from collections import deque
chat_history_queue = deque(maxlen=100)  # 최대 100개 저장
BATCH_SIZE = 10  # 배치 크기
BATCH_INTERVAL = 30  # 30초마다 배치 처리
...
async def batch_save_worker():
    """배치 저장 워커"""
    while True:
        if len(chat_history_queue) >= BATCH_SIZE:
            records = [chat_history_queue.popleft() for _ in range(min(BATCH_SIZE, len(chat_history_queue)))]
...
async def get_conversational_response(query, chat_history):
    ...
    except (IndexError, asyncio.TimeoutError, Exception) as e:
    ...
    with ThreadPoolExecutor() as executor:
        if query_type == "weather":
            ...
            if query_lower in GREETINGS:
                ...
                result = await get_conversational_response(query, st.session_state.messages)
        else:
...
# #         "jobs": "의사, 간호사, 물리치료사, 임상병리사 / 수의사, 동물, 곤충 사육사 / 건축 설계사, 감리사, 측량사, 조경 디자이너/ 천문학자, 지질학자 / 생명공학자, 기계공학자, 생물공학자, 전자공학자 / 약제사, 임상병리사 / 특수작물 재배사,  원예사, 플로리스트"
# #     }
# # }
...
# #     - **추천 직업**: 의사, 간호사, 물리치료사, 임상병리사 / 수의사, 동물, 곤충 사육사 / 건축 설계사, 감리사, 측량사, 조경 디자이너/ 천문학자, 지질학자 / 생명공학자, 기계공학자, 생물공학자, 전자공학자 / 약제사, 임상병리사 / 특수작물 재배사,  원예사, 플로리스트
# # """
...
# #             "5. **공학논문** 📚: '공학논문 [키워드]' (예: 공학논문 Multimodal AI)\n"
# #             "6. **의학논문** 🩺: '의학논문 [키워드]' (예: 의학논문 cancer therapy)\n"
# #             "7. **검색** 🌐: '[키워드] 검색해' 또는 '[키워드] 검색해줘' (예: 4월 전시회 검색해줘)\n"
# #             "8. **MBTI** ✨: 'MBTI 검사',  'MBTI 유형', 'MBTI 설명' (예: MBTI 검사, INTJ 설명)\n"
# #             "9. **다중지능** 🎉: '다중지능 검사', '다중지능 유형', '다중지능 직업', (예: 다중지능 검사, 언어지능 직업)\n\n"
# #             "궁금한 점 있으면 질문해주세요! 😊"
# #         )
