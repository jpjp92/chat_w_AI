...
def save_chat_history(user_id, session_id, question, answer, time_taken):
    # answer가 generator인지 확인하고 처리
    if isinstance(answer, (types.GeneratorType, types.AsyncGeneratorType)):
        logger.error("Attempted to save a generator object to chat history")
        answer = "Generator object cannot be serialized"

    # answer가 DataFrame을 포함하는 경우 직렬화 가능한 형식으로 변환
