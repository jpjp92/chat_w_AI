...
async def process_query(query, messages):
    ...
    with ThreadPoolExecutor() as executor:
        if query_type == "weather":
            future = executor.submit(asyncio.run, weather_api.get_city_weather(extract_city_from_query(query)))
            result = future.result()
            cache_handler.setex(cache_key, 600, result)
            return result, False
        elif query_type == "tomorrow_weather":
            future = executor.submit(asyncio.run, weather_api.get_forecast_by_day(extract_city_from_query(query), 1))
            result = future.result()
            cache_handler.setex(cache_key, 600, result)
            return result, False
        elif query_type == "time":
            if "오늘날짜" in query_lower or "현재날짜" in query_lower or "금일날짜" in query_lower:
                ...
                city = extract_city_from_time_query(query)
                future = executor.submit(asyncio.run, get_time_by_city(city))
                result = future.result()
            cache_handler.setex(cache_key, 600, result)
            return result, False
        elif query_type == "league_standings":
            ...
            if league_key:
                league_info = LEAGUE_MAPPING[league_key]
                future = executor.submit(asyncio.run, football_api.fetch_league_standings(league_info["code"], league_info["name"]))
                result = future.result()
                result = result["error"] if "error" in result else {
                    "header": f"{result['league_name']} 리그 순위",
                    "table": result["data"],
            ...
            if league_key:
                league_info = LEAGUE_MAPPING[league_key]
                future = executor.submit(asyncio.run, football_api.fetch_league_scorers(league_info["code"], league_info["name"]))
                result = future.result()
                result = result["error"] if "error" in result else {
                    "header": f"{result['league_name']} 리그 득점순위 (상위 10명)",
                    "table": result["data"],
            ...
            future = executor.submit(asyncio.run, get_drug_info(query))
            result = future.result()
            cache_handler.setex(cache_key, 600, result)
            return result, False
        elif query_type == "arxiv_search":
            keywords = query.replace("공학논문", "").replace("arxiv", "").strip()
            future = executor.submit(asyncio.run, get_arxiv_papers(keywords))
            result = future.result()
            cache_handler.setex(cache_key, 600, result)
            return result, False
        elif query_type == "pubmed_search":
            keywords = query.replace("의학논문", "").strip()
            future = executor.submit(asyncio.run, get_pubmed_papers(keywords))
            result = future.result()
            cache_handler.setex(cache_key, 600, result)
            return result, False
        elif query_type == "naver_search":
            search_query = query.lower().replace("검색", "").strip()
            future = executor.submit(asyncio.run, get_naver_api_results(search_query))
            result = future.result()
            cache_handler.setex(cache_key, 600, result)
            return result, False
        elif query_type == "mbti":
