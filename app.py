...
class WeatherAPI:
    ...
    def get_city_weather(self, city_name):
        ...
        result = (
            f"{data['name']}의 현재 날씨는 {data['weather'][0]['description']}예요 {weather_emoji}!\n"
            f"온도는 {data['main']['temp']}°C, 체감 온도는 {data['main']['feels_like']}°C 정도네요.\n"
            f"습도는 {data['main']['humidity']}%, 풍속은 {data['wind']['speed']}m/s예요.\n"
            f"오늘 기분이 어떠신가요? 날씨에 맞춰 더 도와드릴까요? 😊"
        )
    ...
    def get_forecast_by_day(self, city_name, days_from_today=1):
        ...
        for forecast in data['list']:
            ...
            if dt == target_date:
                ...
                forecast_text += (
                    f"⏰ {time_only프트} {forecast['weather'][0]['description']} {weather_emoji} "
...
def init_session_state():
    ...
    if "context" not in st.session_state:
        st.session_state.context = {"city": "서울", "intent": None}

# 도시 및 시간 추출 (조사 추가)
CITY_PATTERNS = [
    re.compile(r'(?:오늘|내일|모레|이번 주|주간)?\s*([가-힣a-zA-Z\s]{2,20}(?:시|군|city)?)[의|에|에서]?\s*날씨', re.IGNORECASE),
    ...
    for pattern in CITY_PATTERNS:
        ...
        if match:
            ...
            if city not in ["오늘", "내일", "모레", "이번 주", "주간", "현재"]:
                st.session_state.context["city"] = city  # 맥락 저장
                return city
    return st.session_state.context["city"]  # 기본값으로 이전 도시 사용

TIME_CITY_PATTERNS = [
    re.compile(r'([가-힣a-zA-Z]{2,20}(?:시|군)?)[의|에|에서]?\s*시간'),
    ...
    for pattern in TIME_CITY_PATTERNS:
        ...
        if match:
            ...
            if city != "현재":
                ...
                return city
    return st.session_state.context["city"]
...
def get_kst_time():
    ...
    return f"대한민국 기준: {kst_time.strftime('%Y년 %m월 %d일 %p %I:%M')}예요. ⏰\n\n 더 궁금한 점 있나요? 😊"
...
def get_time_by_city(city_name="서울"):
    ...
    return f"현재 {city_name} 시간은 {city_time.strftime('%Y년 %m월 %d일 %p %I:%M')}예요. ⏰\n\n 더 궁금한 점 있나요? 😊"

# 사용자 및 채팅 기록 관리 (변경 없음, 생략)
...
# 의약품 검색, Naver API, ArXiv, PubMed (변경 없음, 생략)
...
async def get_conversational_response(query, chat_history):
    ...
    messages = [
        {"role": "system", "content": "친절한 AI 챗봇입니다. 자연스럽고 예의 바른 한국어로 대답하며, 적절한 이모지 사용: ✅(완료), ❓(질문), 😊(친절)"},
    ...
    try:
        ...
        result = "응답을 생성하는 중 문제가 발생했어요. 다시 시도해 볼까요? 😓"
    conversation_cache.setex(cache_key, 600, result)
    return result

# GREETING 개선
GREETINGS = ["안녕", "하이", "헬로", "ㅎㅇ", "왓업", "할롱", "헤이"]
GREETING_RESPONSE = "안녕하세요! 반갑습니다. 어떤 도움을 드릴까요? 😊"

# 쿼리 분석 및 의도 분류
def analyze_query(query):
    return okt.pos(query, norm=True, stem=True)

INTENT_MAPPING = {
    "weather": ["날씨", "기온", "습도"],
    "tomorrow_weather": ["내일", "모레"],
    "time": ["시간", "날짜", "몇시"],
    "league_standings": ["리그순위", "순위", "랭킹"],
    "league_scorers": ["리그득점순위", "득점순위", "골"],
    "drug": ["약품검색", "약"],
    "arxiv_search": ["공학논문", "arxiv"],
    "pubmed_search": ["의학논문", "pubmed"],
    "naver_search": ["검색", "찾아"],
...
def needs_search(query):
    ...
    keywords = [word for word, pos in tokens if pos not in ["Josa", "Punctuation"]]  # 조사, 구두점 제외
    
    for intent, intent_keywords in INTENT_MAPPING.items():
        if any(keyword in keywords for keyword in intent_keywords):
            if intent == "weather" and "내일" in keywords:
                return "tomorrow_weather"
            st.session_state.context["intent"] = intent  # 의도 저장
            return intent
    return "conversation"
...
def process_query(query):
    ...
    tokens = analyze_query(query)
    keywords = [word for word, _ in tokens]
    ...
    with ThreadPoolExecutor() as executor:
        if query_type == "weather":
            ...
            if any(kw in keywords for kw in ["오늘", "현재", "금일"]) and "날짜" in keywords:
                result = get_kst_time()
            else:
            ...
            if league_key:
                ...
                result = "지원하지 않는 리그예요. 😓 지원 리그: EPL, LaLiga, Bundesliga, Serie A, Ligue 1"
            ...
            search_query = " ".join(keywords).replace("검색", "").strip()
            future = executor.submit(get_naver_api_results, search_query)
            result = future.result()
        elif query_type == "conversation":
            if any(greeting in keywords for greeting in GREETINGS):
                result = GREETING_RESPONSE
            elif "내일" in keywords and st.session_state.context["intent"] == "weather":
                future = executor.submit(weather_api.get_forecast_by_day, st.session_state.context["city"], 1)
                result = future.result()
            else:
                result = asyncio.run(get_conversational_response(query, st.session_state.chat_history))
        else:
            result = "아직 지원하지 않는 기능이에요. 다른 질문으로 도와드릴까요? 😅"
...
def show_chat_dashboard():
    ...
    if st.button("도움말 ℹ️"):
        st.info(
            "챗봇과 더 쉽게 대화하는 방법이에요! 👇:\n"
            "1. **날씨** ☀️: '[도시명] 날씨' (예: 서울 날씨, 부산에 날씨 어때)\n"
            "2. **시간/날짜** ⏱️: '[도시명] 시간' 또는 '오늘 날짜' (예: 부산 시간, 금일 날짜)\n"
            "3. **리그순위** ⚽: '[리그 이름] 순위' (예: EPL 리그순위, 라리가 득점순위)\n"
    ...
    if user_prompt := st.chat_input("질문해 주세요!"):
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                placeholder.empty()
                error_msg = f"앗, 뭔가 잘못됐어요: {str(e)} 😓 다시 물어보시거나, 조금 다르게 질문해 주세요!"
