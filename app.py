...
class WeatherAPI:
    ...
    def get_city_weather(self, city_name):
        ...
        if not city_info:
            return ["'{city_name}'의 날씨 정보를 가져올 수 없습니다."]
        ...
        self.cache.setex(cache_key, self.cache_ttl, "".join(chunks))
        return chunks
    ...
    def get_forecast_by_day(self, city_name, days_from_today=1):
        ...
        city_info = self.get_city_info(city_name)
        if not city_info:
            return ["'{city_name}'의 날씨 예보를 가져올 수 없습니다."]
        ...
        chunks = [f"{city_info['name']}의 {target_date} 날씨 예보 🌤️\n\n"]
        weather_emojis = {'Clear': '☀️', 'Clouds': '☁️', 'Rain': '🌧️', 'Snow': '❄️', 'Thunderstorm': '⛈️', 'Drizzle': '🌦️', 'Mist': '🌫️'}
        ...
        chunks.append("더 궁금한 점 있나요? 😊") if found else chunks.append(f"'{city_name}'의 {target_date} 날씨 예보를 찾을 수 없습니다.")
        self.cache.setex(cache_key, self.cache_ttl, "".join(chunks))
        return chunks
    ...
    def get_weekly_forecast(self, city_name):
        ...
        city_info = self.get_city_info(city_name)
        if not city_info:
            return ["'{city_name}'의 주간 예보를 가져올 수 없습니다."]
        ...
        chunks = [f"{today_str}({today_weekday_str}) 기준 {city_info['name']}의 주간 날씨 예보 🌤️\n"]
        weather_emojis = {'Clear': '☀️', 'Clouds': '☁️', 'Rain': '🌧️', 'Snow': '❄️', 'Thunderstorm': '⛈️', 'Drizzle': '🌦️', 'Mist': '🌫️'}
...
client = Client(exclude_providers=["OpenaiChat", "Copilot", "Liaobots", "Jmuz", "ChatGptEs"])
...
# 모바일 UI 개선을 위한 CSS (2025-02-26 대화 반영)
st.markdown("""
    <style>
    .stToast {
...
def get_kst_time():
    ...
    chunks = [
        f"대한민국 기준 : {kst_time.strftime('%Y년 %m월 %d일 %p %I:%M')}입니다. ⏰\n",
        "더 궁금한 점 있나요? 😊"
    ]
    return chunks
...
def get_time_by_city(city_name="서울"):
    ...
    chunks = [
        f"현재 {city_name} 시간: {city_time.strftime('%Y년 %m월 %d일 %p %I:%M')}입니다. ⏰\n",
        "더 궁금한 점 있나요? 😊"
    ]
    return chunks
...
def get_drug_info(drug_query):
    ...
    try:
        ...
        if 'body' in data and 'items' in data['body'] and data['body']['items']:
            ...
            cache_handler.setex(cache_key, 86400, "".join(chunks))
            return chunks
...
def get_naver_api_results(query):
    ...
    try:
        ...
        if response.getcode() == 200:
            ...
            chunks.append("더 궁금한 점 있나요? 😊")
            cache_handler.setex(cache_key, 3600, "".join(chunks))
            return chunks
    except Exception as e:
        logger.error(f"Naver API 오류: {str(e)}")
        return [f"검색 중 오류가 발생했습니다: {str(e)} 😓"]

# ArXiv 논문 검색 (스트리밍 시뮬레이션)
...
def needs_search(query):
    query_lower = query.strip().lower().replace(" ", "")  # 띄어쓰기 무시 (2025-03-09)
    ...
    with ThreadPoolExecutor() as executor:
        if query_type == "weather":
            ...
            if specific_type in mbti_descriptions:
                result = split_text_to_chunks(f"### 🎭 {specific_type} 한 줄 설명\n- ✅ **{specific_type}** {mbti_descriptions[specific_type]}")
            else:
                result = split_text_to_chunks(mbti_full_description)
        ...
        cache_handler.setex(cache_key, 600, result if isinstance(result, dict) else "".join(result) if isinstance(result, list) else result)
        return result
